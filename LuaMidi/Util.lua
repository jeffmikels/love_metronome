...
function Util.num_to_var_length(ticks)
   local buffer = bit.band(ticks,0x7F)
   while bit.rshift(ticks,7) > 0 do
      ticks = bit.rshift(ticks, 7)
      buffer = bit.lshift(buffer, 8)
      buffer = bit.bor(buffer, bit.bor(bit.band(ticks, 0x7F), 0x80))
   end
   local buffer_list = {}
   while true do
      buffer_list[#buffer_list+1] = bit.band(buffer, 0xFF)
      if bit.band(buffer, 0x80) > 0 then
         buffer = bit.rshift(buffer, 8)
      else
         break
      end
   end
...
function Util.number_to_bytes(number, bytes_needed)
   ...
   if bit.band(#hex_string, 1) > 0 then
